...
class PlayerManager: NSObject, PlayerViewDelegate {
    ...
    public var isPlayDidEnd = false // 是否播放完成
    ...
    private func initPlayerURL() {
        ...
        do {
            ...
            if #available(iOS 10.0, *) {
               ...
               // _ = try AVAudioSession.sharedInstance().setCategory(AVAudioSession.Category.playback, with: AVAudioSession.CategoryOptions.mixWithOthers)
            }
...
extension PlayerManager {// MARK: 外部调用方法
    ...
    func startPlayAction() {
        /// 重新播放
        if isPlayDidEnd {
            changePlayUrl(playUrlStr!, startTime: 0)
            isPlayDidEnd = false
        }
    }
